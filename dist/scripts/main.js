...
function facebookLoginController($scope, $element, $attrs, $facebook) {
  ...
  $facebook.getLoginStatus().finally().then(function (response) {
...
function createPng(id, callback) {
  ...
  var html = svg.attr('version', 1.1).attr('xmlns', 'http://www.w3.org/2000/svg').node().parentNode.innerHTML;

  var imgsrc = 'data:image/svg+xml;base64,' + btoa(html);

  var canvas = d3.select('body').append('canvas').attr('id', 'canvas-' + id).attr('style', 'display:none').node();
  canvas.width = width;
  ...
  image.onload = function () {
    setTimeout(function () {
      // timeout is needed in Safari to allow the image to render on the page
